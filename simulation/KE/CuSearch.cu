...
int32_t *d_chat, *d_strx, *d_ldrs, *d_candidate;
int *d_hit;
int32_t *d_hw_candidate;
int *d_hw_sizes, *d_hw_offsets;
...
bool load_data(){
    ...
    herr_t status;
    file_id = H5Fopen("../data/attack_data_cs1.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
...
bool load_hw(double sigma, int no){
    hid_t file_id,dataset_id;
    herr_t status;
    for(int q=0;q<4;q++){
        char filename[60];
        ...
        sprintf(filename,"../TA/match_res/match_res_xhat_rk5_no%d_sigma%.1f_%d.h5",q,sigma, no);
        file_id = H5Fopen(filename,H5F_ACC_RDONLY,H5P_DEFAULT);
        if(file_id<0){printf("fail to open file\n"); return false;}
...
void cleanup_gpu_memory(){
    cudaFree(d_chat); cudaFree(d_strx); cudaFree(d_ldrs);
    cudaFree(d_candidate); cudaFree(d_hit);
    cudaFree(d_hw_candidate); cudaFree(d_hw_sizes); cudaFree(d_hw_offsets);
}

__global__ void majority_vote_kernel(
    ...
    int q   = blockIdx.x / N;
    int global_idx = q*N + idx;

    int base_ldrs = q*N*5 + idx*5;
    int hws[5] = {d_ldrs[base_ldrs+0],d_ldrs[base_ldrs+1],d_ldrs[base_ldrs+2],
                   d_ldrs[base_ldrs+3],d_ldrs[base_ldrs+4]};
    ...
    for(int i=0;i<5;i++){
        ...
        for(int j=tid;j<len;j+=blockDim.x){
            ...
            int vote=0;
            int base_strx = global_idx*TEST_TRACE*5;
            for(int t=0;t<trace_num;t++){
...
void majority_vote_gpu(int trace_num){
    ...
    majority_vote_kernel<<<num_blocks,threads_per_block,shared_mem>>>(
        d_chat,d_strx,d_ldrs,trace_num,d_hw_candidate,d_hw_sizes,d_hw_offsets,
        d_candidate,d_hit
    );
...
void dfs_gpu_stats(){
    ...
    int h_hit[TOTAL_COEFF];
    for(int trace_num=1;trace_num<=max_eqs;trace_num++){
        majority_vote_gpu(trace_num);
        cudaMemcpy(h_candidate,d_candidate,TOTAL_COEFF*sizeof(int32_t),cudaMemcpyDeviceToHost);
        cudaMemcpy(h_hit,d_hit,TOTAL_COEFF*sizeof(int),cudaMemcpyDeviceToHost);
        int recovered=0;
...
int main(int argc,char* argv[]){
    if(argc<4){ cout<<"usage: "<<argv[0]<<" <sigma> <no> <max_eqs>\n"; return 1;}
    double sigma = atof(argv[1]);
    int no = atoi(argv[2]);
    max_eqs = atoi(argv[3]);

    if(!load_data()) return 1;
    if(!load_hw(sigma, no)) return 1;
    init_hw_candidate();
    init_gpu_memory();
    dfs_gpu_stats();
    cleanup_gpu_memory();
