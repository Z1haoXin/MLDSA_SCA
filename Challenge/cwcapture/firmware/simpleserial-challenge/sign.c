...
/*************************************************
* Name:        PQCLEAN_MLDSA44_CLEAN_crypto_sign_signature
*
...
rej:
    ...
    shake256_inc_ctx_release(&state);
    PQCLEAN_MLDSA44_CLEAN_poly_challenge(&cp, sig); /* uses only the first SEEDBYTES bytes of sig */
...
/*************************************************
* Name:        PQCLEAN_MLDSA44_CLEAN_crypto_sign
*
...
int PQCLEAN_MLDSA44_CLEAN_crypto_sign(uint8_t *sm,
    ...
    PQCLEAN_MLDSA44_CLEAN_crypto_sign_signature(sm, smlen, sm + PQCLEAN_MLDSA44_CLEAN_CRYPTO_BYTES, mlen, sk);
    *smlen += mlen;
    return 0;
}

/*************************************************
* Name:        PQCLEAN_MLDSA44_CLEAN_crypto_sign_verify
*
...
int PQCLEAN_MLDSA44_CLEAN_crypto_sign_verify(const uint8_t *sig,
    ...
    shake256(mu, CRHBYTES, pk, PQCLEAN_MLDSA44_CLEAN_CRYPTO_PUBLICKEYBYTES);
    shake256_inc_init(&state);
    shake256_inc_absorb(&state, mu, CRHBYTES);
    shake256_inc_absorb(&state, m, mlen);
    shake256_inc_finalize(&state);
    ...
    PQCLEAN_MLDSA44_CLEAN_poly_challenge(&cp, c); /* uses only the first SEEDBYTES bytes of c */
    PQCLEAN_MLDSA44_CLEAN_polyvec_matrix_expand(mat, rho);
    ...
    /* Call random oracle and verify PQCLEAN_MLDSA44_CLEAN_challenge */
    shake256_inc_init(&state);
    shake256_inc_absorb(&state, mu, CRHBYTES);
...
/*************************************************
* Name:        PQCLEAN_MLDSA44_CLEAN_crypto_sign_open
*
...
int PQCLEAN_MLDSA44_CLEAN_crypto_sign_open(uint8_t *m,
    ...
    *mlen = smlen - PQCLEAN_MLDSA44_CLEAN_CRYPTO_BYTES;
    if (PQCLEAN_MLDSA44_CLEAN_crypto_sign_verify(sm, PQCLEAN_MLDSA44_CLEAN_CRYPTO_BYTES, sm + PQCLEAN_MLDSA44_CLEAN_CRYPTO_BYTES, *mlen, pk)) {
        goto badsig;
    } else {
